...
namespace NexusLauncher.UI
{
    public partial class frmTienda : Form
    {
        ...
        private void TryPurchaseGame(Game game)
        {
            ...
            try
            {
                // Verificar si ya es propietario (usar User como primer parámetro)
                ...
                // Agregar a la biblioteca del usuario (pasamos el objeto User según tu GameService)
                var added = _gameService.AddGameToUser(currentUser, game.Id);
                if (added)
                {
                    MessageBox.Show($"Compra realizada: {game.Title}.\nSe agregó a su biblioteca.", "Compra OK", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    btn_Comprar.Enabled = false;
                    btn_Comprar.Text = "Ya comprado";
                }
                else
                {
                    MessageBox.Show("No se pudo completar la compra. Intente nuevamente.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
